...
if st.button("Analyze News"):
    if user_input.strip() == "":
        ...
        # Mapping: Score > 0.55 = REAL NEWS | Score < 0.45 = FAKE NEWS
        if 0.45 <= prediction_prob <= 0.55:
            st.warning(f"⚠️ **UNCERTAIN** (Score: {prediction_prob * 100:.2f}%)\n\n*The text is too short or ambiguous for a confident classification.*")
        elif prediction_prob > 0.55:
            # High output probability (> 0.55) = REAL NEWS
            st.success(f"✅ **REAL NEWS** (Confidence: {prediction_prob * 100:.2f}%)")
        else:
            # Low output probability (< 0.45) = FAKE NEWS
            st.error(f"🚨 **FAKE NEWS** (Confidence: {(1 - prediction_prob) * 100:.2f}%)")